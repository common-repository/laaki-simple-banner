...
class Laaki_Simple_Banner_Widget extends WP_widget {
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'laaki-simple-banner-title' )); ?>"><?php echo __( 'Title:' ); ?></label> 
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'laaki-simple-banner-title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'laaki-simple-banner-title' )); ?>" type="text" value="<?php echo esc_html( $title ); ?>" />
        </p>
       
            <span class="laaki-simple-banner-image" style="width: 100%; display: block">
                <?php if ( $bannerExists ): ?>
                    <img style="width:100%" src="<?php echo esc_url($banner_uri);?>" alt="">
                <?php endif; ?>
            </span>
        ...
        <input 
            type="hidden" 
            id="<?php echo esc_attr($this->get_field_id( 'laaki-simple-banner-uri' )); ?>" 
            name="<?php echo esc_attr($this->get_field_name( 'laaki-simple-banner-uri' )); ?>" 
            value="<?php echo esc_attr( $banner_uri ); ?>"
            class="laaki-simple-banner-uri">

        <?php if ( $bannerExists === true ): ?>        
            <input class="laaki-simple-banner-delete-button button" type="button" value="Delete Image" />
        <?php endif;?>

        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'laaki-simple-banner-link' )); ?>"><?php __( 'Link: [Leave blank for none]' ); ?></label> 
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'laaki-simple-banner-link' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'laaki-simple-banner-link' )); ?>" type="text" value="<?php echo esc_attr( $link ); ?>" />
        </p>
        ...
        <p>
            <input class="" 
                ...
                id="<?php echo esc_attr($this->get_field_id( 'laaki-simple-banner-nofollow' )); ?>" 
                name="<?php echo esc_attr($this->get_field_name( 'laaki-simple-banner-nofollow' )); ?>" /> 

            <label for="<?php echo esc_attr($this->get_field_id( 'laaki-simple-banner-nofollow' )); ?>">No-follow link</label>
        </p>
        <p>
            <input class="" 
                type="checkbox" 
                <?php if ( isset($instance[ 'laaki-simple-banner-open-window' ]) ) checked( $instance[ 'laaki-simple-banner-open-window' ], 'on' ); ?> 
                id="<?php echo esc_attr($this->get_field_id( 'laaki-simple-banner-open-window' )); ?>" 
                name="<?php echo esc_attr($this->get_field_name( 'laaki-simple-banner-open-window' )); ?>" /> 

            <label for="<?php echo esc_url($this->get_field_id( 'laaki-simple-banner-open-window' )); ?>">Open in a new window</label>
        </p>
    ...
    public function widget( $args, $instance ) {
        ...
        if ( strlen($banner_uri) > 0 ):
            if ( strlen( $banner_link ) > 0 ):
                $html .= "<a href='" . esc_url($banner_link) . "' $nofollowTag $openWindowTag>";
                $html .= "<img src='" . esc_url($banner_uri) . "'>";
                $html .= "</a>";
            else:
                $html .=  "<img src='" . esc_url($banner_uri) . "'>";
            endif;
        else:
            $html .=  __('No Image Found!', $this->text_domain);
        endif;
